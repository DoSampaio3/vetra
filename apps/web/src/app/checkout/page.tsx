          ...
          <div style={S.paymentCol}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '16px', marginBottom: '24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'linear-gradient(135deg, #00c6a7, #0099cc)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', flexShrink: 0 }}>🇧🇷</div>
                <div>
                  <div style={{ fontSize: '13px', fontWeight: 700, color: 'white' }}>Powered by Asaas</div>
                  <div style={{ fontSize: '11px', color: 'rgba(100,116,139,0.6)', marginTop: '2px' }}>Plataforma brasileira · Regulada pelo Banco Central</div>
                </div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '12px', color: 'rgba(148,163,184,0.6)' }}>
            ...
            <button onClick={handlePay} disabled={loading} style={{ ...S.payBtn, opacity: loading ? 0.7 : 1, cursor: loading ? 'wait' : 'pointer' }}>
              {loading ? '⏳ Redirecionando...' : `🔒 Pagar com ${PAYMENT_METHODS.find(m => m.key === billingType)?.label} — R$ ${plan.priceValue}`}
            </button>
            <p style={{ fontSize: '11px', color: 'rgba(100,116,139,0.4)', textAlign: 'center', marginTop: '14px' }}>Você será redirecionado para o ambiente seguro do Asaas.</p>
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginTop: '16px', flexWrap: 'wrap' }}>
              {['Visa', 'Mastercard', 'Amex', 'Pix', 'Boleto', 'Elo'].map(c => (
                <span key={c} style={{ fontSize: '10px', color: 'rgba(100,116,139,0.5)', fontFamily: 'monospace', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', padding: '3px 8px', borderRadius: '4px' }}>{c}</span>
              ))}
            </div>
          </div>
        </div>
