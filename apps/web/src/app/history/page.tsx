...
export default function HistoryPage() {
  ...
  const levelBadge: Record<string, any> = {
    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          ...
          <Link href="/verify"><Button size="md">+ Nova Consulta</Button></Link>
        </div>
        ...
        {loadingData ? (
          ...
          <Card>
            <EmptyState icon="◷" title="Nenhuma consulta no histórico"
              description="Suas verificações realizadas aparecerão aqui com resultado e data."
                ...
                <Card key={item.id} hover={false}>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="w-9 h-9 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center flex-shrink-0">
                      <span className="text-sm text-blue-500">{typeIcon[item.query_type] || '🔍'}</span>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                        ...
                        <span className="text-xs text-gray-400 bg-gray-50 px-2 py-0.5 rounded-md border border-gray-100 font-mono">
                          {item.query_type}
                        </span>
                        {item.result_level && (
                          <Badge variant={levelBadge[item.result_level] || 'gray'}>
                            {levelLabel[item.result_level] || item.result_level}
                          </Badge>
                        )}
                      </div>
